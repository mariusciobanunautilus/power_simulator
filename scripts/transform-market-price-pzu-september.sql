/*
 * Power Simulator
 * Transform September 2026 PZU prices into core.market_price
 *
 * Source workbook:
 *   sheet: Septembrie
...
begin
    if exists (
        ...
        raise exception
            'September 2026 PZU prices already exist for run_id = 1. Transformation aborted.';
    end if;
end
$$;

create temporary table tmp_pzu_september_map
on commit drop
as
with price_cells as (
    select
...
mapped as (
    select
        pc.*,
        make_date(2026, 9, pc.day_of_month) as local_date,
        (
            select count(*)
            from core.settlement_interval si
            where si.market_code = 'RO'
              and si.local_date = make_date(2026, 9, pc.day_of_month)
...
begin
    select count(*) into v_source_rows from tmp_pzu_september_map;
    if v_source_rows <> 720 then
        raise exception 'September: expected 720 source cells, found %.', v_source_rows;
    end if;

    select count(*) into v_numeric_rows
    from tmp_pzu_september_map
    where value_text ~ '^-?[0-9]+([.][0-9]+)?$';
    if v_numeric_rows <> 720 then
        raise exception 'September: expected 720 numeric values, found %.', v_numeric_rows;
    end if;
    ...
    if v_formula_rows <> 0 then
        raise exception 'September: expected zero formula cells, found %.', v_formula_rows;
    end if;
    ...
    if v_unique_rows <> 720 then
        raise exception 'September: expected 720 unique mappings, found %.', v_unique_rows;
    end if;
    ...
    if v_zero_candidate_rows <> 0 then
        raise exception 'September: found % zero-candidate mappings.', v_zero_candidate_rows;
    end if;
    ...
    if v_ambiguous_rows <> 0 then
        raise exception 'September: found % ambiguous mappings.', v_ambiguous_rows;
    end if;
    ...
    if v_null_interval_rows <> 0 then
        raise exception 'September: found % null interval mappings.', v_null_interval_rows;
    end if;
    ...
    if v_duplicate_interval_rows <> 0 then
        raise exception 'September: found % duplicate interval mappings.', v_duplicate_interval_rows;
    end if;
end
$$;
...
begin
    ...
    if v_loaded_rows <> 720
       ...
       or v_distinct_intervals <> 720 then
        raise exception
            'September: invalid post-load counts rows %, status %, sources %, intervals %.',
            v_loaded_rows, v_unclassified_rows, v_distinct_source_cells, v_distinct_intervals;
    end if;

    if v_min_price <> (-5.04)::numeric(20,8) then
        raise exception
            'September: unexpected stored minimum %, expected source minimum -5.04 at schema scale.',
            v_min_price;
    end if;

    if v_max_price <> (3236.63)::numeric(20,8) then
        raise exception
            'September: unexpected stored maximum %, expected source maximum 3236.63 at schema scale.',
            v_max_price;
    end if;
end
$$;
