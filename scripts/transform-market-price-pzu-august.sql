/*
 * Power Simulator
 * Transform August 2026 PZU prices into core.market_price
 *
 * Source workbook:
 *   sheet: August
...
begin
    if exists (
        ...
        where mp.run_id = 1
          ...
          and si.local_date >= date '2026-08-01'
          and si.local_date < date '2026-09-01'
    ) then
        raise exception
            'August 2026 PZU prices already exist for run_id = 1. Transformation aborted.';
    end if;
end
$$;

create temporary table tmp_pzu_august_map
on commit drop
as
with price_cells as (
    select
...
mapped as (
    select
        pc.*,
        make_date(2026, 8, pc.day_of_month) as local_date,
        (
            select count(*)
            from core.settlement_interval si
            where si.market_code = 'RO'
              and si.local_date = make_date(2026, 8, pc.day_of_month)
...
begin
    select count(*) into v_source_rows from tmp_pzu_august_map;
    if v_source_rows <> 744 then
        raise exception 'August: expected 744 source cells, found %.', v_source_rows;
    end if;

    select count(*) into v_numeric_rows
    from tmp_pzu_august_map
    where value_text ~ '^-?[0-9]+([.][0-9]+)?$';
    if v_numeric_rows <> 744 then
        raise exception 'August: expected 744 numeric values, found %.', v_numeric_rows;
    end if;
    ...
    if v_formula_rows <> 0 then
        raise exception 'August: expected zero formula cells, found %.', v_formula_rows;
    end if;
    ...
    if v_unique_rows <> 744 then
        raise exception 'August: expected 744 unique mappings, found %.', v_unique_rows;
    end if;
    ...
    if v_zero_candidate_rows <> 0 then
        raise exception 'August: found % zero-candidate mappings.', v_zero_candidate_rows;
    end if;
    ...
    if v_ambiguous_rows <> 0 then
        raise exception 'August: found % ambiguous mappings.', v_ambiguous_rows;
    end if;
    ...
    if v_null_interval_rows <> 0 then
        raise exception 'August: found % null interval mappings.', v_null_interval_rows;
    end if;
    ...
    if v_duplicate_interval_rows <> 0 then
        raise exception 'August: found % duplicate interval mappings.', v_duplicate_interval_rows;
    end if;
end
$$;
...
begin
    ...
    if v_loaded_rows <> 744
       ...
       or v_distinct_intervals <> 744 then
        raise exception
            'August: invalid post-load counts rows %, status %, sources %, intervals %.',
            v_loaded_rows, v_unclassified_rows, v_distinct_source_cells, v_distinct_intervals;
    end if;

    if v_min_price <> (-0.15)::numeric(20,8) then
        raise exception
            'August: unexpected stored minimum %, expected source minimum -0.15 at schema scale.',
            v_min_price;
    end if;

    if v_max_price <> (2276.97)::numeric(20,8) then
        raise exception
            'August: unexpected stored maximum %, expected source maximum 2276.97 at schema scale.',
            v_max_price;
    end if;
end
$$;
